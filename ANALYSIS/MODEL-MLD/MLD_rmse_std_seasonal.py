...
if (len(sys.argv) < 5):
    print ('Usage: '+ sys.argv[0] + ' mip start_year end_year season (winter or summer) exlab(0 or 1)')
    sys.exit()
...
title = [ 'MODEL' , 'STD', 'BIAS' ]
cmap = [ 'RdYlBu_r', 'YlOrBr', 'RdBu_r' ]

if (season == 'winter'):
    co = np.array([0,10,20,50,100,200,300,400,500,600,1000,1500,2000,2500,3000,4000,5000])
    cs = np.array([0,10,20,50,100,200,300,400,500,600,1000])
    cd = np.arange(-200,200,20)
elif (season == 'summer'):
    co = np.arange(0,150,10)
    cs = np.array([0,2,5,10,20,30,50,70,100])
...
for panel in range(3):
    if (panel == 0):
        ...
        da = DS['mld'].std(dim='model',skipna=True,ddof=1)
        ct = cs
        norm = colors.BoundaryNorm(ct,256)
    else:
...
#                         'spacing':'proportional',
    ...
    if (panel == 2):
        ...
        x = DS['lon'].values
        y = DS['lat'].values
        z = np.abs(DS['mldbias'].mean(dim='model',skipna=True)) - 2.0 * DS['mld'].std(dim='model',skipna=True,ddof=1)
