...
fig.suptitle("SST and SSS bias", fontsize=18)
...
axes.text(0.35,0.9,"$r^2$="+'{:.3f}'.format(lr.score(x_np,y_np)))
...
xx=x.reshape(-1,1)
axes.plot(xx,lr.predict(xx),color='red',linewidth=0.25)
axes.text(-0.25,0.2,"$r^2$="+'{:.3f}'.format(lr.score(x_np,y_np)))
...
xx=x.reshape(-1,1)
axes.plot(xx,lr.predict(xx),color='red',linewidth=0.25)
axes.text(-0.15,0.3,"$r^2$="+'{:.3f}'.format(lr.score(x_np,y_np)))
...
plt.subplots_adjust(left=0.07,right=0.75,bottom=0.10,top=0.90,hspace=0.30,wspace=0.25)
