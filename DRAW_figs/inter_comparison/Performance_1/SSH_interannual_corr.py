...
if (sys.argv[1] == 'MMM'):
    suptitle = 'Multi Model Mean' + ' (Correlation of interannual variabilty of SSH from 1993 to 2009)'
    outfile = './fig/zoscor_interannual_MMM'

else:
    suptitle = sys.argv[1] + ' (Correlation of interannual variability of SSH from 1993 to 2009)'
    model_list[0] = [sys.argv[1]]
    model_list[1] = [sys.argv[1]]
    outfile = './fig/zoscor_interannual_' + sys.argv[1]

title = [ '(a) OMIP1', '(b) OMIP2', '(c) OMIP2 - OMIP1', '(d) Taylor diagram' ]

metainfo = [ json.load(open("./json/zoscor_interannual_omip1.json")), 
             json.load(open("./json/zoscor_interannual_omip2.json")) ]
...
bounds2 = [-0.2, -0.15, -0.1, -0.05, -0.02, -0.01, 0.0, 0.01, 0.02, 0.05, 0.1, 0.15, 0.2]
tickbounds1 = [-1.0, -0.7, -0.4, -0.3, -0.2, -0.1, 0.1, 0.2, 0.3, 0.4, 0.7, 1.0]
tickbounds2 = [-0.2, -0.1, -0.02, 0.0, 0.02, 0.1, 0.2]
...
for omip in ['omip1', 'omip2']:
    df=pd.read_csv('csv_zos/zos_' + intvl + '_' + omip + '.csv',index_col=0)
    zosstats += [df]
...
for mip in range(2):
    id_df=id_df+1
    i = 0
    for index, row in zosstats[id_df-1].iterrows():
        print(row[0],row[1])
        if (index != 'Reference'):
            stddev=row[1]*100
            corrcoef=row[0]
...
dia._ax.tick_params(labelsize=8)
...
fig.legend(dia.samplePoints,
           [ p.get_label() for p in dia.samplePoints[0:num_models[0]+num_models[0]+1] ],
