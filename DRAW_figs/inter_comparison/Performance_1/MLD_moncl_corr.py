...
if (sys.argv[1] == 'MMM'):
    ...
    outfile = './fig/mldcor_monclim_MMM'
    ...
    outfile = './fig/mldcor_monclim_' + sys.argv[1]

title = [ '(a) OMIP1', '(b) OMIP2', '(c) OMIP2 - OMIP1', '(d) Taylor diagram' ]

metainfo = [ json.load(open("./json/mldcor_monclim_omip1.json")), 
             json.load(open("./json/mldcor_monclim_omip2.json")) ]
...
for omip in ['omip1', 'omip2']:
    df=pd.read_csv('csv_mld/mld_' + intvl + '_' + omip + '.csv',index_col=0)
    mldstats += [df]
...
rects = dict(interannual=224,
             monclim=224)
legcol = dict(interannual=[0.83,0.05],
              monclim=[0.83,0.05])
