...
if (sys.argv[1] == 'MMM'):
    suptitle = 'Multi Model Mean' + ' (Correlation of monthly climatology of SSH from 1993 to 2009)'
    outfile = './fig/zoscor_monclim_MMM'

else:
    suptitle = sys.argv[1] + ' (Correlation of monthly climatology of SSH from 1993 to 2009)'
    model_list[0] = [sys.argv[1]]
    model_list[1] = [sys.argv[1]]
    outfile = './fig/zoscor_monclim_' + sys.argv[1]

title = [ '(a) OMIP1', '(b) OMIP2', '(c) OMIP2 - OMIP1', '(d) Taylor diagram' ]

metainfo = [ json.load(open("./json/zoscor_monclim_omip1.json")), 
             json.load(open("./json/zoscor_monclim_omip2.json")) ]
