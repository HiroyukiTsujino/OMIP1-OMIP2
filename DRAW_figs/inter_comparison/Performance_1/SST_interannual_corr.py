...
if (sys.argv[1] == 'MMM'):
    suptitle = 'Multi Model Mean' + ' (Correlation of interannual variabilty of SST from 1980 to 2009)'
    outfile = './fig/toscor_interannual_MMM'
else:
    suptitle = sys.argv[1] + ' (Correlation of interannual variability of SST from 1980 to 2009)'
    model_list[0] = [sys.argv[1]]
    model_list[1] = [sys.argv[1]]
    outfile = './fig/toscor_interannual_' + sys.argv[1]


title = [ '(a) OMIP1', '(b) OMIP2', '(c) OMIP2 - OMIP1', '(d) Taylor diagram' ]

metainfo = [ json.load(open("./json/toscor_interannual_omip1.json")), 
             json.load(open("./json/toscor_interannual_omip2.json")) ]
