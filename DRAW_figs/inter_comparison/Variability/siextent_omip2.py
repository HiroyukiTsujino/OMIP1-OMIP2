...
metainfo=json.load(open('json/siextent_omip2.json'))
lineinfo=json.load(open('../json/inst_color_style.json'))

outfile = './fig/Fig1e_omip2.png'
...
siextentn_mar_clean.plot(y=siextentn_mar_clean.columns[i],ax=axes,ylim=[0,20],color='darkgrey',linewidth=4,title='Sea Ice extent NH OMIP2 (JRA55-do) Mar and Sep')
ii=0
for inst in metainfo.keys():
...
siextentn_sep_clean.plot(y=siextentn_sep_clean.columns[i],ax=axes,ylim=[0,20],color='darkgrey',linewidth=4,label='_nolegend_')
ii=0
for inst in metainfo.keys():
    coltmp=lineinfo[inst]["color"]
    stytmp=lineinfo[inst]["style"]
...
siextents_mar_clean.plot(y=siextents_mar_clean.columns[i],ax=axes,ylim=[0,27],color='darkgrey',linewidth=4,title='Sea Ice extent SH OMIP2 (JRA55-do) Mar and Sep')
ii=0
for inst in metainfo.keys():
    coltmp=lineinfo[inst]["color"]
    stytmp=lineinfo[inst]["style"]
