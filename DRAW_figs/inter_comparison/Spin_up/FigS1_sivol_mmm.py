...
title_list = [ "(a) NH", "(b) SH" ]


metainfo = [ json.load(open("./sivol_omip1_full.json")),
             json.load(open("./sivol_omip2_full.json")) ]
...
nf = 0
for var in var_list:
    DS[0][var].mean(dim='model').plot(ax=ax[nf],color='darkred',label='OMIP1')
    ax[nf].fill_between(x=DS[0]['time'],
                        y1=DS[0][var].min(dim='model'),
                        y2=DS[0][var].max(dim='model'),
                        alpha=0.5, facecolor='lightcoral')
    DS[1][var].mean(dim='model').plot(ax=ax[nf],color='darkblue',label='OMIP2')
...
for m in range(2):
    ...
    ax[m].legend(bbox_to_anchor=(1.15,1.0))
    ...
    ax[m].set_ylabel(r'$10^{3} \mathrm{km}^{3}$',fontsize=12)
