...
outfile = './fig/FigS2_profile_1d_OMIP' + str(omip) + '-' + var_list[item]

suptitle = 'OMIP-' + str(omip) + ' horizontal mean  (vertical 1d) ' + var_list[item] + ' first year'
...
#J 参照データ読込
d_woa = np.empty( (2,12,33) )

print( "Loading WOA13v2 data" )

reffile = '../refdata/WOA13v2/1deg_L33/monthly/woa13_decav_th.1000'
da_ref = xr.open_dataset(reffile,decode_times=False)["thetao"]

wgt0 = np.empty((180,360))
for j in range(180):
    wgt0[j] = math.cos(math.radians(da_ref.lat.values[j]))

wgt = np.tile( wgt0, (12,33,1,1) ) * np.logical_not(np.isnan(da_ref))
d_woa[0] = np.average(da_ref.fillna(0), weights=wgt, axis=(2,3))

#print(d_woa[0])

reffile = '../refdata/WOA13v2/1deg_L33/monthly/woa13_decav_s.1000'
da_ref = xr.open_dataset(reffile,decode_times=False)["so"]

wgt = np.tile( wgt0, (12,33,1,1) ) * np.logical_not(np.isnan(da_ref))
d_woa[1] = np.average(da_ref.fillna(0), weights=wgt, axis=(2,3))
    ...
    for var in var_list:
        ...
        for model in model_list[n]:
            ...
            if (n == 1) and (model == "FSU-HYCOM"):
                ...
                d_fsu[isto:iedo+1,:] = d_tmp[istf:iedf+1,:] * factor
                #print (d_fsu)
                d[nvar,nmodel] = d_fsu - d_woa[nvar].mean(axis=0)
                #d[nvar,nmodel] = d_fsu
                ...
                d[nvar,nmodel] = d_coco - d_woa[nvar].mean(axis=0)
                ...
                d[nvar,nmodel] = d_cmcc - d_woa[nvar].mean(axis=0)
                ...
                d[nvar,nmodel] = d_barca - d_woa[nvar].mean(axis=0)
                ...
                d[nvar,nmodel] = d_gfdl - d_woa[nvar].mean(axis=0)
...
#                    DS_read = xr.open_mfdataset(infile,decode_times=False,concat_dim='time')
                ...
                d[nvar,nmodel] = DS_read[var].values * factor - d_woa[nvar].mean(axis=0)
...
for model in model_list[omip-1]:
    ...
    if (omip == 1):
        da.isel(model=nmodel,time=0).plot(y='depth',ax=ax[nmodel])
    else:
        da.isel(model=nmodel,time=1).plot(y='depth',ax=ax[nmodel])

    ax[nmodel].plot(d_woa[item].mean(axis=0)-d_woa[item,0,:],lev33)
    ax[nmodel].set_title(model,{'fontsize':9,'verticalalignment':'top'})
    ax[nmodel].tick_params(labelsize=7)
    ax[nmodel].set_xlim(-0.4,0.4)
    ax[nmodel].set_xlabel('')
    ax[nmodel].set_ylabel('')
    ax[nmodel].set_yticks(np.linspace(0,6000,13))
...
plt.subplots_adjust(left=0.08,right=0.98,bottom=0.05,top=0.93,hspace=0.26)
