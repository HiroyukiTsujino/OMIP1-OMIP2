...
suptitle = 'OMIP2 sensitivity to formulae of moist air'
metainfo = json.load(open("./json/circ_omip2_aircore.json"))
...
for nvar in range(4):
    ...
    if ( nvar == 3 ):
        ax[nvar].set_xlabel('year',fontsize=12)
    else:
        ax[nvar].set_xlabel('',fontsize=12)

    ax[nvar].grid()
    ax[nvar].set_ylim(ylim[nvar])
    ax[nvar].set_yticks(ytick[nvar])
    ax[nvar].set_ylabel(r'$10^{9} \mathrm{kg}\,\mathrm{s}^{-1}$',fontsize=12)
