...
if (int(sys.argv[1]) == 3):
    metainfo = [ json.load(open("./vat_omip1_full.json")),
                 json.load(open("./vat_omip2_full.json")) ]
    suptitle = 'Vertically averaged temperature anomaly (Multi Model Mean)'
    outfile = './fig/FigS1_vatanom_MMM.png'

else:
    metainfo = [ json.load(open("./vat_omip1.json")),
                 json.load(open("./vat_omip2.json")) ]
    suptitle = 'Vertically averaged temperature anomaly (OMIP' + sys.argv[1] +')'
    outfile = './fig/FigS1_vatanom_OMIP'+sys.argv[1]+'.png'
...
for n in range(2):
    ...
    for nv in range(nvar):
        for nm in range(nmodel):
            d_new[nv,nm,:] = d_new[nv,nm,:] - d_new[nv,nm,nst]
...
if ( n < 2 ):
    if len(model_list[n]) > 1:
        ...
        for var in var_list:
            ...
            ax[nf].legend(bbox_to_anchor=(1.2,1.0))
            nf += 1
...
ylim = [ [-1.0, 1.0], [-0.8, 0.8], [-0.8, 0.8], [-0.8, 0.8] ]
#    ylimr= [ [12.0, 28.0], [12.0, 28.0] ]
ytick = [ np.linspace(-1.0, 1.0, 21),
...
for m in range(4):
    ...
    if ( m == 3 ):
        ax[m].set_xlabel('year',fontsize=12)
    else:
        ax[m].set_xlabel('',fontsize=12)

    ax[m].grid()
    ax[m].set_ylim(ylim[m])
    ax[m].set_yticks(ytick[m])
    ax[m].set_ylabel(r'$^{\circ}$C',fontsize=12)

#for m in range(2):
#    ax_r[m].set_ylim(ylimr[m])
#    ax_r[m].set_yticks(ytickr[m])
