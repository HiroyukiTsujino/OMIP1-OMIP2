...
axes0 = np.array( [ [0.05, 0.75, 0.07, 0.17],
                    [0.13, 0.75, 0.10, 0.17],
                    [0.24, 0.75, 0.08, 0.17], ])
...
if (nv_out == 4):
    outfile = './fig/MOC_'+ item[nv_out] + '_shallow.png'
else:
    outfile = './fig/MOC_'+ item[nv_out] + '.png'
...
for m in range(3):
    ...
    da.isel(basin=m).plot.contour(ax=ax[11][m],
                                  colors='black',
                                  levels=bounds,
                                  add_labels=False,
                                  linewidths=1.0)
...
for model in model_list[0]:
    ...
    for m in range(3):
        if ((nmodel == 0) and (m == 0)):
            da.isel(basin=m).plot(ax=ax[nax][m],cmap=cmap[nv_out],
                                  levels=bounds,
                                  extend='both',
                                  cbar_kwargs={'orientation': 'vertical',
                                               'spacing':'uniform',
        ...
        da.isel(basin=m).plot.contour(ax=ax[nax][m],
                                      colors='black',
                                      levels=bounds,
                                      add_labels=False,
                                      linewidths=1.0)

        ax[nax][m].set_title(title[m],{'fontsize':6,'verticalalignment':'top'})
        ax[nax][m].tick_params(labelsize=7)
        ax[nax][m].invert_yaxis()
        ax[nax][m].set_xlim(xlim[m][0],xlim[m][1])
        ax[nax][m].set_xticks(np.arange(xlim[m][0],xlim[m][1]+0.1,30))
        ax[nax][m].set_facecolor('lightgray')
    for m in range(1,3):
        ax[nax][m].tick_params(axis='y',labelleft=False)


    #if (nmodel == 6):
    #    nax = nax + 2 # skip BSC-NEMO
    #else:
    nax = nax + 1
