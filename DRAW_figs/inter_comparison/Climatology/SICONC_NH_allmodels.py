...
suptitle = head_title[nv_out]  + ' SICONC NH ' + month[mon_cal-1] + ' (ave. from '+str(ystr)+' to '+str(yend)+')'
...
outfile = './fig/SICONC_NH_allmodels_'+item[nv_out]+'_'+month[mon_cal-1]+'.png'
...
daamip = DS['amip'].sel(lat=slice(40.5,89.5)).mean(dim='time')
...
da = DS[item[nv_out]].sel(lat=slice(40.5,89.5)).mean(dim='time').mean(dim='model',skipna=False)
...
ax[nax].set_extent([-180,180,40,90], ccrs.PlateCarree())
...
for model in model_list[0]:
    ...
    da = DS[item[nv_out]].isel(model=nmodel).sel(lat=slice(40.5,89.5)).mean(dim='time')
    ...
    ax[nmodel].set_extent([-180,180,40,90], ccrs.PlateCarree())
