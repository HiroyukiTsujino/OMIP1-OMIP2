...
print(DS0)
danh = DS0.mlotst.sel(time=slice('1850-07-01','1850-09-01'),lat=slice(0.0,90.0)).mean(dim='time')
dash = DS0.mlotst.sel(time=slice('1850-01-01','1850-03-01'),lat=slice(-90.0,0.0)).mean(dim='time')
...
for omip in range(2):
    ...
    for model in model_list[omip]:
        ...
        tmp = DS.mlotst.sel(time=slice('1850-07-01','1850-09-01'),lat=slice(0.5,89.5)).mean(dim='time')
        dnh[nmodel] = tmp.values
        tmp = DS.mlotst.sel(time=slice('1850-01-01','1850-03-01'),lat=slice(-89.5,-0.5)).mean(dim='time')
...
axes0 = np.array( [ [0.05, 0.80, 0.4, 0.13],
                    [0.05, 0.67, 0.4, 0.13],])

ax = [ [ plt.axes(axes0[0],projection=proj),
         plt.axes(axes0[1],projection=proj), ],
       [ plt.axes(axes0[0]+np.array([0.46,0,0,0]),projection=proj),
         plt.axes(axes0[1]+np.array([0.46,0,0,0]),projection=proj), ],
       [ plt.axes(axes0[0]+np.array([0,-0.31,0,0]),projection=proj),
         plt.axes(axes0[1]+np.array([0,-0.31,0,0]),projection=proj),],
       [ plt.axes(axes0[0]+np.array([0.46,-0.31,0,0]),projection=proj),
         plt.axes(axes0[1]+np.array([0.46,-0.31,0,0]),projection=proj), ],
       [ plt.axes(axes0[0]+np.array([0,-0.62,0,0]),projection=proj),
         plt.axes(axes0[1]+np.array([0,-0.62,0,0]),projection=proj), ],
       [ plt.axes(axes0[0]+np.array([0.46,-0.6215,0,0]),projection=proj),
         plt.axes(axes0[1]+np.array([0.46,-0.62,0,0]),projection=proj), ] ]

# [left, bottom, width, height]
ax_cbar = [
    plt.axes([0.92,0.68,0.012,0.23]),
    plt.axes([0.92,0.37,0.012,0.23]),
    plt.axes([0.92,0.06,0.012,0.23]),
]
...
for panel in range(6):
    for ns in range(2):
        ...
        if ns == 0:
            ...
            ax[panel][ns].text(155,  6,"JAS", size = 12, bbox = boxdic)
        else:
            ...
            #h = h + 0.03
            #ax[panel][ns].set_position([l,b,w,h])
            ax[panel][ns].text(155,-84,"JFM", size = 12, bbox = boxdic)
