...
if len(sys.argv) == 1:
    suptitle = 'Multi Model Mean' + ' (MLD March ave. from 1980 to 2009)'
    outfile = './fig/MLD_MAR.png'
else:
    suptitle = sys.argv[1] + ' (MLD March ave. from 1980 to 2009)'
    model_list[0] = [sys.argv[1]]
    model_list[1] = [sys.argv[1]]
    outfile = './fig/MLD_MAR_' + sys.argv[1] + '.png'
...
print(DS0)
da0 = DS0.mlotst.sel(time='1850-03-01')
...
for omip in range(2):
    ...
    for model in model_list[omip]:
        ...
        tmp = DS.mlotst.sel(time='1850-03-01')
        d[nmodel] = tmp.values
        nmodel += 1
