...
!
!====================================================
  use libmxe_para, only: pi, radius, radian, radian_r, dbl
  implicit none
...
!====================================================
  subroutine mp2lp(lambda, phi, mu0, psi0)
    ...
    REGION: if(psi0 <= NPLAT*radian_r) then
      ...
      zeta=cmplx(tmp1,tmp2,dbl)  !  mu,psi から zeta
      !
...
!====================================================
  subroutine rot_mp2lp(rot_cos,rot_sin,lambda0, phi0, mu0, psi0)
    ...
    if ( phi0 == 0.5d0 * pi ) then
      write(6,*) 'rot_mp2lp error: phi0 =',phi0
      stop
    endif
    !
    REGIN: if(psi0 <= NPLAT*radian_r) then
      ...
      tmp1=dtan(0.5d0*psi)*dcos(mu)
      tmp2=dtan(0.5d0*psi)*dsin(mu)
      zeta=cmplx(tmp1,tmp2,dbl)  !  mu0, psi から zeta
      ...
      z=cmplx(tmp1,tmp2,dbl)  !  lambda0, phi から z
      !
