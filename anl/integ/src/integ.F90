...
function integ__area(d,k,cgrid,para,grid,topo,istr,iend,jstr,jend,lcore)
  implicit none
  ...
  logical,intent(in),optional :: lcore

  real(8) :: f
  integer :: i,j,i0,i1,j0,j1
  real(8),allocatable :: area(:,:)
  real(8) :: areat(para%imut,para%jmut)
  ...
  if ( cgrid=='U' ) then
    ...
    call make_areat( para, grid, topo, k, areat )
    area(i0:i1,j0:j1) = areat(i0:i1,j0:j1)
  endif
  ...
  if ( cgrid=='U' ) then
    ...
    do j = j0, j1
      do i = i0, i1
        f = f + d(i,j) * area(i,j) * grid%nendidx(j)
      enddo
    enddo
  end if
  deallocate(area)
...
function integ__vol(d,cgrid,para,grid,topo,istr,iend,jstr,jend,kstr,kend,lcore)
  implicit none
  ...
  if ( cgrid == 'U' ) then
    ...
    do k = k0, k1
      ...
      do j = j0, j1
        do i = i0, i1
          f = f + d(i,j,k) * topo%dzt(i,j,k) * areat(i,j) * grid%nendidx(j)
        enddo
      enddo
    enddo
  endif
...
subroutine make_areat( para, grid, topo, k_arg, area )
  implicit none
  ...
  real(8),               intent(out):: area(para%imut,para%jmut)

  integer         :: i, j

  i = 1
  j = 1
  area(i,j) = topo%aexl(i,j,k_arg) * grid%a_bl(i,j) 

  i = 1
  do j = 2, para%jmut
    area(i,j) = topo%aexl(i,j,k_arg) * grid%a_bl(i,j) &
         & + topo%aexl(i,j-1,k_arg) * grid%a_tl(i,j-1) 
  enddo

  j = 1
  do i = 2, para%imut
    area(i,j) = topo%aexl(i,j,k_arg) * grid%a_bl(i,j) &
         & + topo%aexl(i-1,j,k_arg) * grid%a_br(i-1,j) 
  enddo
  ...
  do j = 2, para%jmut
    do i = 2, para%imut
      area(i,j) = topo%aexl(i,j,k_arg) * grid%a_bl(i,j) &
           & + topo%aexl(i-1,j,k_arg) * grid%a_br(i-1,j) &
           & + topo%aexl(i,j-1,k_arg) * grid%a_tl(i,j-1) &
           & + topo%aexl(i-1,j-1,k_arg) * grid%a_tr(i-1,j-1) 
    enddo
  enddo

end subroutine make_areat
!--------------------------------------------------------------
